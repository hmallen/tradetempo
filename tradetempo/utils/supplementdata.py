...
class SupplementData:
    def __init__(self, db, collection, process_new=False):
        ...
        insert_result = self.db[config['mongodb']['exchange_collection']].insert_many(exchanges.json()['result'])
        logger.debug(f"Added {len(insert_result.upserted_ids) if insert_result.upserted_ids is not None else 0} exchanges.")
        markets = requests.get('https://api.cryptowat.ch/markets').json()['result']
        insert_result = self.db[config['mongodb']['market_collection']].insert_many(markets.json()['result'])
        logger.debug(f"Added {len(insert_result.upserted_ids) if insert_result.upserted_ids is not None else 0} markets.")
